    ...
    try {
        ...
        connection.query(mandatoryQuery, [surveyId], (err, mandatoryQuestions) => {
            ...
            const queries = answers.map(response => {
                let { questionId, answer, actionType } = response;
 
                if (!questionId || !actionType) {
                    console.error("Invalid data detected. Missing questionId or actionType:", response);
                    return Promise.reject(new Error('Missing questionId or actionType'));
                } 
                ...
                if (actionType === "I") {
                    ...
                    return new Promise((resolve, reject) => {
                        connection.query(insertQuery, [surveyId, questionId, answer, updatePerson], (err, result) => {
                            if (err) {
                    ...
                    const updateQuery = `
                        ...
                        SET ANSWER = ?, UPDATE_TIMESTAMP = NOW()
                        WHERE SURVEY_ID = ? AND QUESTION_ID = ? AND UPDATE_PERSON = ?
                    `;
                    return new Promise((resolve, reject) => {
                        connection.query(updateQuery, [answer, surveyId, questionId,updatePerson], (err, result) => {
                            if (err) {
                    ...
                    return Promise.reject(new Error(`Invalid actionType: ${actionType}`));
                }
            });
 
            Promise.all(queries)
                .then(() => {
                    const checkSurveyStatusQuery = `
                        SELECT COUNT(*) AS totalMandatory, 
                    ...
                    connection.query(checkSurveyStatusQuery, [surveyId, surveyId, updatePerson], (err, results) => {
                        ...
                        const updateSurveyStatusQuery = `
                            UPDATE survey_response 
                            SET ANSWER_STATUS = 'completed' 
                            WHERE SURVEY_ID = ? AND UPDATE_PERSON = ?
                        `;
